...
The agent presets system (`docs/dev/notes/agent-presets.md`) exposes context-aware Space capabilities as MCP tools for opencode. This design covers the next layer: runtime agents that use those capabilities, LLM providers, local deterministic functions, and other agents to perform tasks for the user.
...
- [Agent Presets Design](/dev/notes/agent-presets) - context-aware capability and MCP tool exposure
- [Remote MCP Integration](/dev/notes/remote-mcp) - OpenCode MCP transport
- [Codex SDK For Fennel](/dev/notes/codex-sdk-fennel) - Codex handle, streaming, and cancellation shape
