...
- [ ] **Step 1: Write failing tests for explicit non-sandbox scene access.** In `test-sandbox-scene-world-data.fnl`, create a fake world with `activity.sessions.sandbox.scene` and `activity.sessions.graph.scene`. Give each session distinct panels, terrains, lights, skybox, and background. Add assertions that `WorldData.scene-state-panels manager "test-world" "graph"`, `WorldData.terrain-state-records manager "test-world" "graph"`, and `WorldData.scene-state-background manager "test-world" "graph"` read graph session data.
