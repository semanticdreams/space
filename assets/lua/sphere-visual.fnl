...
(local SphereMesh (require :sphere-mesh))
...
(fn style-cache-key [options]
  (.. "sphere|segments="
      (tostring (math.max 3 (math.floor (or options.segments 24))))
      "|rings="
      (tostring (math.max 2 (math.floor (or options.rings 16))))
      "|color="
      (SphereMesh.vec4-key (or options.color (glm.vec4 0.5 0.75 1.0 1.0)))))
...
(fn ensure-shared-batch [ctx options]
  ...
  (SharedInstancedMeshCache.acquire
    ...
    key
    (fn []
      (local mesh (SphereMesh.build-mesh options))
