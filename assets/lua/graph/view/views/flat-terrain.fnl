...
(fn FlatTerrainNodeView [node opts]
  ...
  (fn build [ctx]
    ...
    (local info-label
      ((Text {:text (.. "Editing flat terrain " (or (and target target.terrain-id) "?"))})
       build-ctx))
    ...
    (fn update-status-and-actions []
      ...
      (local error-count (count-errors errors))
    ...
    (fn refresh-from-target [opts]
      (local refresh-options (or opts {}))
      (set syncing? true)
    ...
    (local apply-button
      ((Button {:text "Apply"
                ...
                :on-click (fn [_button _event]
                            (local validation (Validation.validate-draft draft))
                            (set applied? false)
                            (set apply-failed? false)
                            (each [_ spec (ipairs Validation.field-specs)]
                              (local message (. validation.errors spec.key))
                                ...
                                (do
                                  ...
                                  (local updated
                                    (if (and target target.apply-values)
                                        (target:apply-values validation.values)
                                        nil))
                                  (when (not updated)
