...
(fn NotebooksNodeView [node opts]
  ...
  (fn build [ctx]
    ...
    (assert build-ctx "NotebooksNodeView requires a build context")
    (local view {})
    ...
    (local create-button
      ((Button {:icon "add"
                ...
                :on-click (fn [_button _event]
                            (when (and target target.create-notebook target.add-notebook-node)
                              (local notebook (target:create-notebook {}))
                              (target:add-notebook-node notebook)))})
       build-ctx))

    (local search
      ((SearchView {:items []
                    :name "notebooks-node-view"
                    :num-per-page 10
                    :builder (fn [item child-ctx]
                               ...
                               ((Button {:text label
                                         :variant :ghost
                                         :on-click (fn [_button _event]
                                                     (when (and target target.add-notebook-node)
                                                       (target:add-notebook-node (. item 1))))})
                                child-ctx))})
       build-ctx))
    ...
    (search.submitted:connect
      (fn [item]
        (when (and target target.add-notebook-node item)
          (target:add-notebook-node (. item 1)))))
