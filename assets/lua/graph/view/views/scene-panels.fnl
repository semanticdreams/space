...
(fn ScenePanelsNodeView [node opts]
  ...
  (fn build [ctx]
    ...
    (assert build-ctx "ScenePanelsNodeView requires a build context")
    (local view {})
    (local search
      ((SearchView {:items []
                    :name "scene-panels-node-view"
                    :num-per-page 10
                    :builder (fn [item child-ctx]
                               ...
                               ((Button {:text label
                                         ...
                                         :on-click (fn [_button _event]
                                                     (when (and target target.add-panel-node)
                                                       (target:add-panel-node (. item 1))))})
                                child-ctx))})
       build-ctx))
    (local flex
      ((Flex {:axis 2
    ...
    (search.submitted:connect
      (fn [item]
        (when (and target target.add-panel-node item)
          (target:add-panel-node (. item 1)))))
    (view:refresh-items)
    view))
ScenePanelsNodeView
