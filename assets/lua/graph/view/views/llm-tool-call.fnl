...
(fn LlmToolCallView [node opts]
    (assert node "LlmToolCallView requires a node")
    (local options (or opts {}))
    (local target (or node options.node))

    (fn build [ctx]
        (local build-ctx (or ctx options.ctx (and target target.graph target.graph.ctx)))
        (assert build-ctx "LlmToolCallView requires a build context")
        ...
        (local rows
            [{:key :name
             ...
             {:key :arguments
              :label "Arguments"
              :value (and target target.arguments)
              :multiline? true
              :min-lines 3
              :max-lines 10}])
