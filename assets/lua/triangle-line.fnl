...
(fn ensure-glm-vec4 [value fallback]
    (if (and value
        ...
        value
        (= (type value) :table)
        (glm.vec4 (or (. value 1) 0.4) (or (. value 2) 0.4) (or (. value 3) 0.4) (or (. value 4) 1.0))
        fallback))

(fn TriangleLine [ctx opts]
    (assert ctx "TriangleLine requires a build context")
    (local vector (and ctx ctx.triangle-vector))
    (assert vector "TriangleLine requires ctx.triangle-vector")
    (local options (or opts {}))
    (var color (ensure-glm-vec4 options.color (glm.vec4 0.4 0.4 0.4 1)))
    ...
    (fn write-batch-data [self start-pos end-pos]
        (assert start-pos "TriangleLine update missing start position")
        (assert end-pos "TriangleLine update missing end position")
    ...
    (fn set-color [self new-color]
        (set color (ensure-glm-vec4 new-color color))
