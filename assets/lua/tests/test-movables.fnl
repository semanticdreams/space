...
(fn movables-stop-drag-when-pointer-target-disabled []
  (with-camera (glm.vec3 0 0 -1)
    (fn []
      ...
      (movables:register widget {:key widget
                                 ...
                                 :on-drag-end (fn [_entry]
                                                (set ended true))})
      (set intersector.selection-point (glm.vec3 1 2 3))
      (set intersector.next-ray {:origin (glm.vec3 7 8 10)
                                 :direction (glm.vec3 0 0 -1)})
      (movables:on-mouse-button-down {:button 1 :x 0 :y 0})
      (set enabled? false)
      (movables:on-mouse-motion {:x 20 :y 20})
      (set app.pointer-target-enabled? original-enabled)
      (assert (approx layout.position.x 4)
              "Disabled pointer target drag should not update x")
      (assert (approx layout.position.y 5)
              "Disabled pointer target drag should not update y")
      (assert (approx layout.position.z 6)
              "Disabled pointer target drag should not update z")
      (assert ended "Disabled pointer target drag should end the active drag")
      (assert (not (movables:drag-engaged?))
              "Disabled pointer target drag should clear drag state"))))
