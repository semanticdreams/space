...
(fn make-test-ctx []
  ...
  (set ctx.get-text-ssbo-batcher
    ...
    (set ctx.clickables (assert (make-clickables-stub) "graph view control test context requires clickables"))
    (set ctx.hoverables (assert (make-hoverables-stub) "graph view control test context requires hoverables"))
