...
(fn resize-dialog [ctx]
  ...
  (local dialog-builder
    (Dialog {:title "Resize Direct"
             :child (fn [child-ctx]
                      ((Text {:text "Right drag"
  ...
  (let [(manual projected world-pos) (screen-point-for-layout hud layout offset)]
    ...
    (let [(hit-manual _point-manual _distance-manual) (layout:intersect ray-manual)
      ...
      (local payload-up {:button 3
                         :x end.x
                         :y end.y})
      (app.resizables:on-mouse-button-down payload-down)
      (assert (and app.resizables (app.resizables:drag-engaged?))
              "HUD resize expected to engage resizables")
      (app.resizables:on-mouse-motion payload-move)
      (app.resizables:on-mouse-button-up payload-up)
      (hud:update)))
  ...
  (Harness.capture-snapshot {:name "hud-float-resize-direct"
                             :width ctx.width
                             :height ctx.height
                             :tolerance 3})
  hud)
...
(fn main []
  (Harness.with-app {}
                   (fn [ctx]
                     (run ctx)))
  (print "E2E hud float resize direct snapshot complete"))
