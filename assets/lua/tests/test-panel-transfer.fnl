...
(fn setup-app-stubs []
  (local clickables {:register (fn [_self] nil)
  ...
  (local hoverables {:register (fn [_self] nil) :unregister (fn [_self] nil)})
  (assert clickables.register "panel-transfer setup requires clickable register stub") (assert hoverables.register "panel-transfer setup requires hoverable register stub")
...
(fn restore-app-state-restores-nil-interaction-context []
  (local original (save-app-state))
  (set app.clickables nil)
  (set app.hoverables nil)
  (local saved (save-app-state))
  (setup-app-stubs)
  (local (ok err) (pcall restore-app-state saved))
  ...
  (set app.launcher original.launcher)
  ...
  (when (not ok)
    (error err))
  (assert (= saved.clickables nil) "test requires nil saved clickables")
  (assert (= saved.hoverables nil) "test requires nil saved hoverables"))
(table.insert tests {:name "restore app state restores nil interaction context" :fn restore-app-state-restores-nil-interaction-context})
