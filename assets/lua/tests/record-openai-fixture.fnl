...
(fn wait-until [pred timeout-ms]
  (callbacks.run-loop {:poll-jobs false
                        :poll-http true
                        :sleep-ms 10
                        :timeout-ms (if timeout-ms timeout-ms 60000)
                        :until pred}))
...
(fn main []
  ...
  (local engine (EngineModule.Engine {:headless true}))
  (set app.engine engine)
  (local (ok result) (pcall record-fixture))
  ...
  (when (not cleanup-ok)
    (error cleanup-err))
  (print "Recorded OpenAI fixture" result))
