...
(fn run-with-clean-signal [body]
  (local signal (Signal))
  (local (ok result) (pcall (fn [] (body signal))))
  (cleanup signal)
  (when (not ok)
    (error result))
  result)
...
(fn signal-disconnect-during-emit-skips-later-listener []
  (run-with-clean-signal
    (fn [signal]
      ...
      (local late-listener (fn [_value] (table.insert seen "late")))
      (local last-listener (fn [_value] (table.insert seen "last")))
      (signal:connect first-handler)
      (set late-handler (signal:connect late-listener))
      (signal:connect last-listener)
      (signal:emit "x")
      (assert (= (length seen) 2))
      (assert (= (. seen 1) "first"))
      (assert (= (. seen 2) "last"))
    ...
    (fn [signal]
      ...
      (local first-handler
        (fn [_value]
          (table.insert seen "first")
          (signal:clear)))
      (local second-handler (fn [_value] (table.insert seen "second")))
      (signal:connect first-handler)
      (signal:connect second-handler)
      (signal:emit "x")
      (signal:emit "y")
...
(fn signal-connect-during-emit-only-affects-future-emits []
  (run-with-clean-signal
    (fn [signal]
      (local seen [])
      (var late-calls 0)
      (local late-listener (fn [_next] (set late-calls (+ late-calls 1))))
      (local first-listener
        (fn [_value]
          (table.insert seen "first")
          (signal:connect late-listener)))
      (signal:connect first-listener)
      (signal:emit "x")
      (assert (= late-calls 0)
