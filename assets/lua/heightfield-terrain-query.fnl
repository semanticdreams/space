...
(fn chunk-samples [record]
  (local samples (or (and record record.options record.options.chunk-samples) [17 17]))
  [(or (. samples 1) samples.x 17)
   (or (. samples 2) samples.y samples.z 17)])
...
(fn build-chunk-map [record]
  ...
  (each [_ chunk (ipairs (or record.chunks []))]
    ...
    (set (. out (chunk-key chunk-x chunk-z)) chunk))
  out)

(fn sample-height-global [record chunk-map sample-x sample-z]
  (local sample-counts (chunk-samples record))
  (local stride-x (- (. sample-counts 1) 1))
  (local stride-z (- (. sample-counts 2) 1))
  (local primary-chunk-x (floor-div sample-x stride-x))
  (local primary-chunk-z (floor-div sample-z stride-z))
  ...
  (fn try-sample [chunk-x chunk-z local-x local-z]
    (local chunk (. chunk-map (chunk-key chunk-x chunk-z)))
    (if (and chunk
