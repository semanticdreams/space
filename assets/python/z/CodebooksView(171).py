class CodebooksView:
    def __init__(self):
        self.name = z.ReactiveValue('codebooks')
        ...
        self.column = z.Flex([self.head, self.search_view], axis='y', xalign='largest')
    ...
    def search_view_item_builder(self, item, context):
        b = z.ContextButton(label=item[1], focus_parent=context['focus_parent'], color=(0.4, 0.9, 0.4, 1),
                   actions=[
                       ('get', lambda: world.floaties.add(CodebookView(item[0]))),
                   ])
        return b

    def load_items(self):
        self.items = world.codebooks.get_codebooks()
        self.items_updated()

    def items_updated(self):
        self.search_view.set_items([(x, x.data['name'] or '') for x in self.items])

    def create_item(self):
        item = world.codebooks.create_codebook()
