...
(global get-db (fn []
  (assert appdirs "appdirs module is missing")
  (local data-dir (appdirs.user-data-dir "space"))
  (local db_path (fs.join-path data-dir "app.engine.db"))
  (when (and fs fs.parent fs.create-dirs)
    (fs.create-dirs (fs.parent db_path)))
  (sqlite.open db_path)))
...
;(-> G (. :force-layout-params) (pp))
  ...
  ;  (set app.engine.world (world-entity-func (G:make-this world-entity))))
